...
class DatabaseProvider implements IDBProvider {
  ...
  Future<T> getPageDataFromDB<T>(
      Function dataParseFunc, String collectionName, String docName) {
    Completer<T> pageCompleter = Completer();
    var db = ref.read(firebaseDbInstanceProvider);

    db.collection(collectionName).doc(docName).get().then((pageDoc) {
      T pageDocData =
          dataParseFunc(pageDoc.data() as Map<String, dynamic>) as T;
